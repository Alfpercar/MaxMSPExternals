// filedate.c -- manage search paths
...
int main()
{
	setup((t_messlist **)&filedate_class, (method)filedate_new,0L, (short)sizeof(t_filedate), 0L, 0);
	addmess((method)filedate_anything, "anything", A_GIMME, 0);
	addmess((method)filedate_assist,"assist",A_CANT,0);
	
	finder_addclass("System","filedate");
...
{	
	...
	if (!locatefile_extended(filename,&vol,&type,&type,-1)) {	// look for all files
...
void filedate_out(t_filedate *x, unsigned long date)
{
	...
	SETLONG(list,dtr.month);
	SETLONG(list+1,dtr.day);
	SETLONG(list+2,dtr.year);
	SETLONG(list+3,dtr.hour);
	SETLONG(list+4,dtr.minute);
	SETLONG(list+5,dtr.second);
...
void *filedate_new(void)
{
	...
	x = (t_filedate *)newobject(filedate_class);

...
void jslider_assign(t_jslider *x, double f);
...
int main()
{
	...
	CLASS_ATTR_CHAR(c, "floatoutput", 0, t_jslider, j_floatoutput);
...
void *jslider_new(t_symbol *s, short argc, t_atom *argv)
{
	...
	x->j_floatoutput = -1;	// this makes sure that we don't clip the size until the floatoutput attribute is set.
...
void jslider_set(t_jslider *x, t_symbol *s, long argc, t_atom *argv)
{
	if (argc && (argv->a_type == A_FLOAT || argv->a_type == A_LONG))
		jslider_assign(x, atom_getfloat(argv)); 
}
...
t_max_err jslider_setvalueof(t_jslider *x, long ac, t_atom *av)
{
	if (ac && av) {
		if (av->a_type == A_FLOAT)
			jslider_float(x, atom_getfloat(av));
		else if (av->a_type == A_LONG)
			jslider_int(x, atom_getlong(av));
	}
	return MAX_ERR_NONE;
}

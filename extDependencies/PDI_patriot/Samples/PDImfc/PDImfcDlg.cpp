...
CPDImfcDlg::OnRawDataReady( WPARAM wP, LPARAM lP )
{
	...
	m_csFCount.Format("%d", m_dwContFrameCount);
	UpdateData(FALSE);

	if ((PBYTE)lP == g_pMotionBuf)
	{
		CString msg;
		msg.Format("Wrapping at frame %d\r\n", m_dwContFrameCount );
		AddMsg(msg);
	}
...
CPDImfcDlg::ParseFrame( PBYTE pBuf, DWORD dwSize, BOOL bToFile/*=FALSE*/ )
{
    ...
    while ( i<dwSize)
    {
        ...
        msg.Format( "Sensor %d: %d %f %f %f %f %f %f\r\n", ucSensor, fc, x, y, z, az, el, ro );
		if (bFile)
		{
			file.Write( (PVOID)(LPCSTR)msg, msg.GetLength() );
		}
		else
		{
			AddMsg(msg);
	...
	if (bFile)
	{
		...
		if (::AfxMessageBox("View File?", MB_YESNO) == IDYES)
		{
			CString str;
			str.Format("notepad.exe %s", (LPCSTR)fdlg.GetPathName() );
...
{
	TRACE("CPDImfcDlg::OnDataStarted\n");
    CString     msg;
...
CPDImfcDlg::StartCaptureTimer( VOID )
{
	...
	TRACE("CPDImfcDlg::StartCaptureTimer: %x\r\n", m_uIDTimer );
	if (!m_uIDTimer)
	{
		CString msg;
		msg.Format("Timer error!\r\n");
		AddMsg( msg );
		StopCont();
	}
}
...
{
	TRACE("CPDImfcDlg::OnDataStopped\n");
    CString     msg;

	CTimeSpan  ts = m_timeStop - m_timeStart;
	CString    s = ts.Format("%M:%S");
	msg.Format("WM_PI_DATA_STOPPED: time elapsed %s,  %d frames collected, %d bytes, %d frames dropped, %d frames displayed\r\n", 
				(LPCSTR)s, m_dwContFrameCount, m_dwContSize, m_dwOverflowCount, m_dwFramesDisplayed);
	AddMsg(msg);
...
{
    CString     msg;
	msg.Format("WM_PI_RAWDATA_ERROR received: %s\r\n", 
...
{
    CString     msg;
	msg.Format("WM_PI_RAWDATA_WARNING received: %s\r\n", 
		m_pdiDev.ResultStr( (ePiErrCode)wP, (ePiDevError)lP ));
	AddMsg(msg);
...
VOID CPDImfcDlg::StartStyPno( VOID )
{
	...
	if (!m_bCont)
	{
		...
		{
			msg.Format("Stylus Station %d is not an active Station.", m_nStylus);
		...
		msg.Format("SetStylusMode %d PI_STYMODE_POINT", m_nStylus );
		AddResultMsg(msg);
	}
...
CPDImfcDlg::ClearStyMode( VOID )
{
	...
	AddResultMsg("SetSStylusMode -1 PI_STYMODE_MOUSE");
}
...
VOID
CPDImfcDlg::AddResultMsg( LPCSTR szCmd )
{
	CString msg;

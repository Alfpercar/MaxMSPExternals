...
//
//  VSS $Header: /PIDevTools/Inc/PDIdev.h 31    12/12/06 3:19p Suzanne $  
//
/////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////
#ifndef _PDIDEV_H_
...
public:
	...
	BOOL	ClearSBoresight	( INT nStation );					// UN-Boresight station ('^B' command )
	BOOL	EnableStation	( INT nStation, BOOL bEnable );		// Enables or disables station ( '^U' Command )

	BOOL	GetSDataList	( INT nStation, CPDImdat &   );		// Gets motion data output list per station ( 'O' Command )
	BOOL	GetSAlignment	( INT nStation, PDI3vec & O, 
											PDI3vec & X, 
											PDI3vec & Y );		// Gets Alignment Reference Frame per station ( 'A' Command )
	BOOL	GetSBoresight	( INT nStation, PDIori & );			// Get boresight per station ( 'B' Command )
	BOOL	GetSHemisphere	( INT nStation, PDI3vec & );		// Get Hemisphere Of Operation per station ( 'H' Command )
	BOOL	GetSHemiTrack	( INT nStation, BOOL & );			// Query Hemisphere Tracking per station ('H' Command )
	BOOL	GetSStylusMode	( INT nStation, ePiStylusMode & );	// Get Stylus Mode per station ( 'L' Command )

	BOOL	ResetSAlignment ( INT nStation );					// Reset Alignment per station ( '^R' Command )

	BOOL	SetSDataList	( INT nStation, const CPDImdat &   );			// Sets motion data output list per station ( 'O' Command )
	BOOL	SetSAlignment	( INT nStation, const PDI3vec & O, 
											const PDI3vec & X, 
											const PDI3vec & Y );			// Sets Alignment Reference Frame per station ( 'A' Command )
	BOOL	SetSBoresight	( INT nStation, const PDIori &, BOOL bReset=FALSE );	// Set boresight per station ( 'B' Command )
	BOOL	SetSHemisphere	( INT nStation, const PDI3vec & );				// Set Hemisphere Of Operation per station ( 'H' Command )
	BOOL	SetSHemiTrack	( INT nStation );								// Enable Hemisphere Tracking per station ('H' Command )
	BOOL	SetSStylusMode	( INT nStation, ePiStylusMode );				// Set Stylus Mode per station ( 'L' Command )
	...
	// Host Prediction 
	BOOL	SetHostPrediction	( const DWORD & dwMap, const INT & nMS );	// Enables host prediction by station, sets level

	BOOL	ResetTracker	( VOID );							// Initializes Tracker, disconnects ( '^Y' Command )
	BOOL	TxtCmd	( LPCSTR szCmd, DWORD & dwRspSize, LPCSTR szRsp=0 );	// Sends generic text command to device
